...
public class DatabaseOperations {
	...
	public static SessionFactory createSessionFactory() {
		...
		configurationObject.addAnnotatedClass(com.pack.xmlops.Employee.class);
	...
	public static void addGivenEmployee(Employee e) {

		Employee empObject=null;
		try {
			System.out.println("Inside addGivenEmployee");
			sessionObject=createSessionFactory().openSession();
			System.out.println("Inside addGivenEmployee---->> 2");
			sessionObject.beginTransaction();
				empObject= new Employee();
				empObject.setEmpId(e.getEmpId());
				empObject.setEmpFName(e.getEmpFName());
				empObject.setEmpLName(e.getEmpLName());
				empObject.setTechnology(e.getTechnology());
				empObject.setExperience(e.getExperience());
				sessionObject.save(empObject);
				//sessionObject.save(e);
				sessionObject.getTransaction().commit();
				logger.info("Successfully created employee data record");
		} catch(Exception e1) {
			e1.printStackTrace();
